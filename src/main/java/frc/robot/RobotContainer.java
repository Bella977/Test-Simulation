...
public class RobotContainer {
    ...
    private double MaxAngularRate = RotationsPerSecond.of(0.001).in(RadiansPerSecond); // 3/4 of a rotation per second max angular velocity
    ...
    private void configureBindings() {
        ...
        if(DriverStation.isTeleop()){
            ...
            drivetrain.applyRequest(() ->
                drive.withVelocityX(-joystick.getLeftX() * MaxSpeed*0.7* (-1)) // Drive forward with negative Y (forward)
                    .withVelocityY(-joystick.getLeftY() * MaxSpeed*0.7) // Drive left with negative X (left)
